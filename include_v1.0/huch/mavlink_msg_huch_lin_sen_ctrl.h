...
#define MAVLINK_MSG_ID_HUCH_LIN_SEN_CTRL 226
...
#define MAVLINK_MSG_ID_HUCH_LIN_SEN_CTRL_LEN 9
...
#define MAVLINK_MESSAGE_INFO_HUCH_LIN_SEN_CTRL { \
	...
	{  { "msec", NULL, MAVLINK_TYPE_UINT32_T, 0, 0, offsetof(mavlink_huch_lin_sen_ctrl_t, msec) }, \
         { "arg", NULL, MAVLINK_TYPE_UINT32_T, 0, 4, offsetof(mavlink_huch_lin_sen_ctrl_t, arg) }, \
         { "cmd", NULL, MAVLINK_TYPE_UINT8_T, 0, 8, offsetof(mavlink_huch_lin_sen_ctrl_t, cmd) }, \
         } \
}
...
static inline uint16_t mavlink_msg_huch_lin_sen_ctrl_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint32_t msec, uint8_t cmd, uint32_t arg)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[9];
	_mav_put_uint32_t(buf, 0, msec);
	_mav_put_uint32_t(buf, 4, arg);
	...
	return mavlink_finalize_message(msg, system_id, component_id, 9, 17);
}
						           ...
						           uint32_t msec,uint8_t cmd,uint32_t arg)
{
	...
	msg->msgid = MAVLINK_MSG_ID_HUCH_LIN_SEN_CTRL;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 9, 17);
}
...
static inline void mavlink_msg_huch_lin_sen_ctrl_send(mavlink_channel_t chan, uint32_t msec, uint8_t cmd, uint32_t arg)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[9];
	_mav_put_uint32_t(buf, 0, msec);
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_HUCH_LIN_SEN_CTRL, buf, 9, 17);
#else
	mavlink_huch_lin_sen_ctrl_t packet;
	packet.msec = msec;
	packet.arg = arg;
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_HUCH_LIN_SEN_CTRL, (const char *)&packet, 9, 17);
#endif
}
 ...
 */
static inline uint32_t mavlink_msg_huch_lin_sen_ctrl_get_arg(const mavlink_message_t* msg)
{
...
#else
	memcpy(huch_lin_sen_ctrl, _MAV_PAYLOAD(msg), 9);
#endif
}
